    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            if transition == "LA":
                ...
                self.stack.append(word1)
            else:
                self.dependencies.append((word2, word1))
                self.stack.append(word2)
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses):
        ...
        for i in range(len(transition_batch)):
            parse_batch[i].parse_step(transition_batch[i])
        unfinished_parses = [parse for parse in partial_parses if len(parse.stack) > 1 or len(parse.buffer) >= 1]
