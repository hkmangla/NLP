...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        if window_size == 1:
            newSen.insert(0,start)
            newSen.append(end)
        else:
            newSen[0:0] = start
            newSen.extend(end)
        for i in range(1,len(newSen)-1):
            features = []
            for w in range(2*window_size+1):
                features += newSen[i - window_size + w] 
            windowed_data.append((features,labels[i-1]))
    ### END YOUR CODE
...
def test_make_windowed_data():
    ...
    a = sum(len(sentence) for sentence in sentences)
